...
## 2. 输入信息
以下是当前的路由上下文信息（已通过动态提示词注入）：
- **用户消息**：{user_message}
- **项目阶段**：{project_stage}（discovery/research/unknown）
- **当前活跃智能体**：{current_agent}（如果没有则为 None）
...
```json
{
  ...
  "reason": "路由原因说明",
  "confidence": 0.0-1.0,
  "instruction": "传递给子智能体的任务指令（基于用户消息生成，简洁明确）",
}
```

### instruction 生成规则：
- **新需求/新想法**：基于用户消息生成简洁的任务指令，例如："用户希望基于原来的需求，新增一个想法"
- **继续当前阶段**：使用用户消息作为 instruction，或生成简短的继续指令
- **修改需求**：基于用户消息生成修改指令，例如："用户希望修改需求，添加新功能：xxx"
